...
class Core extends Proto {
   ...
   public function add()
   {
      ...
      if ($this->checkAuth())
      {
         ...
         if ($_POST)
         {
            ...
            $this->redirect('/grabber/?mod=grabber');
         }
   ...
   protected function _form($values = array())
   {
      ...
      $this->page .= '
         ...
         <div class="location"><a href="/grabber/?mod=grabber">Grabber</a> / Добавление/редактирование</div>
   ...
   public function getUsers()
   {
      ...
      {
         $this->page .= '<form action="/grabber/?mod='.MODULE.'&action=email" method="post">';
   ...
   public function getList()
   {
      ...
      $this->page .= '<div class="location">'.ucfirst(MODULE).' / Типы поиска <a style="margin-left:20px" href="/grabber/?mod=grabber&amp;action=add">добавить</a></div>';
      ...
      while ($row = mysql_fetch_object($query))
      {
         ...
         $this->page .= '<td class="sm"><h3>'.($total_cars > 0 ? '<a href="/grabber/?mod='.MODULE.'&amp;action=grid&amp;target='.$row->id.'">'.$row->name.'</a>' : $row->name).'</h3>';
         //$this->page .= 'присвоено поисков: '.$this->_stat($row->id);
         $this->page .= '<p><a href="/grabber/?mod=grabber&amp;action=edit&amp;id='.$row->id.'" title="Редактировать"><img src="/img/edit.png" alt="" /></a>';
         $this->page .= '<a style="margin-left:5px" href="/grabber/?mod=grabber&amp;action='.($row->active ? 'stop' : 'start').'&amp;id='.$row->id.'" title="'.($row->active ? 'Stop' : 'Start').'"><img src="/img/'.($row->active ? 'stop' : 'start').'.png" alt="" /></a>';
         $this->page .= '</p></td>';
   ...
   public function getGrid()
   {
     ...
     <script type="text/javascript" src="/js/carlist.js"></script>';
         $this->page .= '<div class="location"><a href="/grabber/?mod='.MODULE.'">'.ucfirst(MODULE).'</a> / '.$name.'</div>';
         $this->page .= '<table width="970" border="0" cellspacing="0" cellpadding="0" class="list vlines">';
         $this->page .= '<tr class="title sortButtons">';
         ...
         while ($row = mysql_fetch_object($query))
         {
            ...
            $this->page .= '</td>
               <td class="sm recommend"><a href="/grabber/?mod='.MODULE.'&action=recommend&id='.$row->id.'" title="Рекомендовать"><img src="/img/recommend.png" /></a></td>
            </tr>';
   ...
   public function sorterTD($module, $item, $caption)
   {
      ...
      return '<td onMouseOver="this.className=\'sortButtonsHover\'" onMouseOut="this.className=\'\'" onclick="document.location=\'/grabber/?'.$qs.'\'">'.$caption.($_GET['sortby'] == $item ? $this->sortImg(strtolower($order)) : '').'</td>';
   }

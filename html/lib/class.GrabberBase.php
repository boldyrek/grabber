...
abstract class Proto {
	...
	public static function mDump($var, $varDump = false, $exit = true) {
	    echo '<pre>';
		if ($varDump) {
		    var_dump($var);
        } else {
    ...
    protected function initLang(){
        if(isset($_SESSION['lang']))
        {
            ...
            if($this->user_lang=='eng') {
                $this->translate = new Zend_Translate('gettext',$_SERVER['DOCUMENT_ROOT'].'/inc/langs/en.mo','en');
			} elseif($this->user_lang=='rus') {
                $this->translate = new Zend_Translate('gettext',$_SERVER['DOCUMENT_ROOT'].'/inc/langs/ru.mo','ru');
			}
...
/*
    ...
    private function logoff() {
        ...
        /*Убираем куки*/
        setcookie("elmundo", null, time()-1);
        setcookie("teqiero", null, time()-1);
        setcookie("muchachos", null, time()-1);
        setcookie("acc_id", null, time()-1);
    ...
    public function makeTopMenu()
    {
        ...
        if($this->permissions['type']!='0' and $this->permissions['type']!='')
        {
            ...
            if($_SESSION['user_type']!='8' && $_SESSION['user_type']!='9' && $_SESSION['user_type']!='14')
            {
               $this->page .= '<td><dl>';
            ...
            if ($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7')
            {
               ...
               <td>
                  <dl>
					 ...
					 <dd><a href="'.$this->root_path.'?mod=findnear">'.$this->translate->_('уровень снега').'</a></dd>
                  </dl>
               </td>
               <td>
                  <dl>

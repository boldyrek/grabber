...
class Filecache {
    ...
    public function import($html = FALSE, $is_oodle = FALSE, $is_autosalon = FALSE)
    {	
        // cache already exists
        if (FALSE === $this->_path) return TRUE;
        ...
        foreach ($matches AS $path)
        {
            ...
            if (trim($path) != '')
            {
                ...
                if ($is_oodle) {
					...
					if ($is_autosalon) {
					    if (strstr($path_src, '?') !== FALSE) {
						    $parts = explode('?', $path_src);
							if (isset($parts[1])) {
							    $q = $parts[1];
							} else {
							    $q = '';
						...
						$dest = $dest.($q != '' ? '.'.md5($q).'.jpg' : '');
						
						if (strstr($source, '?') === FALSE) $source = $source.($q != '' ? '?'.$q : '');
					}
                    $rep_cache[$path_src] = $dest;
                ...
                if (! file_exists($dest))
                {
				    if (stristr($source, 'https://') === false) {
                        $result_copy[] = @copy($source, $dest);
					} else {
					    $result_copy[] = $this->copySecureFile($source, $dest);
					}
                }
            }
        }
        ...
        $result_put = @file_put_contents($filename, $html);

  		/*
		echo '<pre>';
		var_dump(array($filename, mb_strlen($html), $result_copy));
		echo '</pre>';
		*/

...
class Source_AutoSalon extends Source implements Kohana_Source {
   ...
   public function execute()
   {
      ...
      {          
         ...
         $fields = $condition['fields'] + array(
            ...
            'fromOdometer' => intval($car->mileage) == 0 ? 'ALL' : intval($car->mileage)
         );
         ...
         while (true) {
			 ...
			 if ($first_pass) {
			     ...
			     $params_list = $this->_build_params_list($fields).'&'. 
								...
								$this->_build_params_list($fields_offset, array('recordOffset', 'searchResultsOffset'), $offset_cnt*$search['offset']);
			 }
             ...
             {
                if ((bool) preg_match('#<td><b>Всего результатов:</b> (\d+)</td>#iu', $response_basic, $matches)) {
   ...
   public function callBack($matches) {
	   ...
	   preg_match("#<img width='60' src='([^']*)'[^>]*>#isU", $matches[1], $found);
	   if (isset($found[1])) {
	       $image_present = 'parsers/manheim_com/images/no-image.jpg' == $found[1] ? 0 : 1;
	   } else {
	       $image_present = 0;
	   }
	   
	   preg_match("#<td width='40%'><b><a href='([^']*)'[^>]*>([^<]*)</a></b>#isU", $matches[1], $found);
	   ...
	   preg_match("#<br>([0-9A-Z]{17})#U", $matches[1], $found);
	   if (isset($found[1])) {
	       $vin_code = trim($found[1]);
	   } else {
	       $vin_code = '';
	   }
	   
	   preg_match("#<br><b>Пробег:</b>([^<]*)#isu", $matches[1], $found);
	   if (isset($found[1])) {
	   ...
	   preg_match("#<span class='small2'>([^|]*)[|]([^<]*)</span>#isu", $matches[1], $found);
	   if (isset($found[1]) && isset($found[2])) {
	   ...
	   preg_match("#<b>Цена прогноз:</b>[^>]*>([^<]*)</a>#isu", $matches[1], $found);
	   if (isset($found[1])) {
   ...
   {     
      ...
      try
      {
	     ...
	     $pattern  = "#<div class='copart_hr'></div><table width='100%'>(.*)</table>#isU";
   ...
   protected function _picture_exists($url, $vincode)
   {
      ...
      if ($picture_exists)
      {
         Filecache::factory()
            ->set_url($url)
            ->set_path(strtoupper($vincode).'-AUTOSALON')
